...
public class BatchResponseTest {
    ...
    public void testBatchResponse() throws BatchException, IOException {
        ...
        parts.add(BatchResponsePart.responses(responses)
                                   .changeSet(false)
                                   .build());

        ODataResponse changeSetResponse = ODataResponse.status(HttpStatusCodes.NO_CONTENT)
                                                       .build();
        ...
        assertEquals(202, batchResponse.getStatus()
                                       .getStatusCode());
    ...
    public void testTwoChangeSetResponse() throws BatchException, IOException {
        ...
        ODataResponse changeSetResponse = ODataResponse.status(HttpStatusCodes.NO_CONTENT)
                                                       .build();
        ODataResponse changeSetResponseTwo = ODataResponse.status(HttpStatusCodes.NO_CONTENT)
                                                          .build();
        List<ODataResponse> responses = new ArrayList<ODataResponse>(1);
        responses.add(changeSetResponse);
    ...
    public void testBatchResponseRawBytesAsStream() throws IOException, ODataException {
        ...
        assertEquals(202, batchResponse.getStatus()
                                       .getStatusCode());
        assertNotNull(batchResponse.getEntity());
        BatchLineReader reader = new BatchLineReader(batchResponse.getEntityAsStream());
        ...
        String contentHeader = batchResponse.getContentHeader();
        BatchParser parser = new BatchParser(contentHeader, true);
        List<BatchSingleResponse> result = parser.parseBatchResponse(new ByteArrayInputStream(builder.toString()
                                                                                                     .getBytes("iso-8859-1")));
        assertEquals(1, result.size());
        assertArrayEquals(data, result.get(0)
                                      .getBody()
    ...
    public void testissueWithHeaderHavingUmlautChars() throws BatchException, IOException {
        ...
        assertNotNull(batchResponse.getEntity());
        assertTrue(batchResponse.getEntity()
                                .toString()
