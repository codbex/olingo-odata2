...
public class AtomServiceDocumentConsumer {
        ...
        try {
            ...
            if (workspaces.isEmpty()) {
                throw new EntityProviderException(
                        EntityProviderException.INVALID_STATE.addContent("Service element must contain at least one workspace element"));
            }
            reader.close();
            atomInfo.setWorkspaces(workspaces)
    ...
    private CommonAttributesImpl parseCommonAttribute(final XMLStreamReader reader) {
        ...
        for (int i = 0; i < reader.getAttributeCount(); i++) {
            if (((!FormatXml.XML_BASE.equals(reader.getAttributeLocalName(i)) || !Edm.PREFIX_XML.equals(reader.getAttributePrefix(i))) && (!FormatXml.XML_LANG.equals(reader.getAttributeLocalName(i)) || !Edm.PREFIX_XML.equals(reader.getAttributePrefix(i))) && (!"local".equals(reader.getAttributeNamespace(i)) && !DEFAULT_PREFIX.equals(reader.getAttributePrefix(i))))) {
                extAttributes.add(new ExtensionAttributeImpl().setName(reader.getAttributeLocalName(i))
    ...
    private WorkspaceImpl parseWorkspace(final XMLStreamReader reader) throws XMLStreamException, EntityProviderException {
        ...
        return new WorkspaceImpl().setTitle(title)
    ...
    private CollectionImpl parseCollection(final XMLStreamReader reader) throws XMLStreamException, EntityProviderException {
        ...
        return new CollectionImpl().setHref(resourceIdentifier)
                                   .setTitle(title)
                                   .setCommonAttributes(attributes)
                                   .setExtesionElements(extElements)
                                   .setAcceptElements(acceptList)
    ...
    private AcceptImpl parseAccept(final XMLStreamReader reader) throws XMLStreamException {
        ...
        return new AcceptImpl().setCommonAttributes(commonAttributes)
                               .setText(text);
    }
    ...
    private ExtensionElementImpl parseExtensionSansTitleElement(final XMLStreamReader reader)
        ...
        if ((!Edm.NAMESPACE_APP_2007.equals(reader.getNamespaceURI()) && (!FormatXml.ATOM_TITLE.equals(reader.getLocalName()) || !Edm.NAMESPACE_ATOM_2005.equals(reader.getNamespaceURI())))) {
    ...
    private ExtensionElementImpl parseElement(final XMLStreamReader reader) throws XMLStreamException, EntityProviderException {
        ...
        ExtensionElementImpl extElement = new ExtensionElementImpl().setName(reader.getLocalName())
                                                                    ...
                                                                    .setPrefix(reader.getPrefix());
        extElement.setAttributes(parseAttribute(reader));
        while (reader.hasNext() && (!reader.isEndElement() || (extElement.getName() == null) || !extElement.getName()
        ...
        if (extElement.getText() == null && extElement.getAttributes()
                                                      .isEmpty()
                && extElement.getElements()
                             .isEmpty()) {
